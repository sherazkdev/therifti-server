...
class TokenServices {
    ...
    */
    async VerifyResetToken(resetTokenObject) {
        ...
        if (token.isUsed)
            throw new ApiError(STATUS_CODES.NOT_FOUND, ERROR_MESSAGES.AUTH.TOKEN_IS_USED);
        if (token.expiresAt.getTime() < now)
            throw new ApiError(STATUS_CODES.NOT_FOUND, ERROR_MESSAGES.AUTH.TOKEN_EXPIRED);
