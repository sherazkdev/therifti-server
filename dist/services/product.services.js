...
class ProductServices {
    ...
    */
    async SearchProduct(searchDetails) {
        ...
        if (price) {
            /** Sort min price */
            if (price.min)
                searchProductQuery.price.$gte = Number(price.min);
            /** Sort min price */
            if (price.max)
                searchProductQuery.price.$lte = Number(price.max);
        }
        if (categoryId)
            searchProductQuery.categoryId = new mongoose.Types.ObjectId(categoryId);
        if (conditions && conditions.length > 0)
            searchProductQuery.condition = { $in: conditions };
        if (sizes && sizes.length > 0)
            searchProductQuery.size = { $in: sizes };
        if (materials && materials.length > 0)
            searchProductQuery.meterial = { $in: materials };
        if (brands && brands.length > 0)
            searchProductQuery.brand = { $in: brands };
        if (q)
            searchProductQuery.title = { $regex: q, $options: "i" };
    ...
    async GetProductById(productId) {
        ...
        if (!product)
            throw new ApiError(STATUS_CODES.NOT_FOUND, ERROR_MESSAGES.PRODUCT.NOT_FOUND);
        return product;
    }
    ;
    /**
     ...
     */
    async GetFeaturedProducts(object) {
        ...
        if (price) {
            /** Sort min price */
            if (price.min)
                featuredProductsQuery.price.$gte = Number(price.min);
            /** Sort min price */
            if (price.max)
                featuredProductsQuery.price.$lte = Number(price.max);
        }
        if (sort) {
        ...
        if (categoryId)
            featuredProductsQuery.categoryId = new mongoose.Types.ObjectId(categoryId);
        if (categoryId)
            featuredProductsQuery.size = { $in: sizes };
        if (categoryId)
            featuredProductsQuery.categoryId = new mongoose.Types.ObjectId(categoryId);
     ...
     */
    async GetSingleProductById(singleDataObject) {
