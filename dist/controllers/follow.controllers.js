...
class FollowControllers {
    followServices = new FollowServices();
    /**
     ...
     */
    HandleFollowSeller = async (req, res) => {
        ...
        const followSellerPayload = { ...result.data, followingId: req.user._id.toString() };
        await this.followServices.FollowSeller(followSellerPayload);
        return res.status(STATUS_CODES.OK).json(new ApiResponse([], SUCCESS_MESSAGES.FOLLOW.FOLLOWED, true, STATUS_CODES.OK));
    };
    /**
    ...
    HandleUnfollow = async (req, res) => {
        ...
        const unFollowSellerPayload = { ...result.data, followingId: req.user._id.toString() };
        await this.followServices.UnFollowSeller(unFollowSellerPayload);
        return res.status(STATUS_CODES.ACCEPTED).json(new ApiResponse([], SUCCESS_MESSAGES.FOLLOW.UNFOLLOWED, true, STATUS_CODES.ACCEPTED));
    };
    /**
     ...
     */
    HandleGetFollowers = async (req, res) => {
        ...
        return res.status(STATUS_CODES.OK).json(new ApiResponse(followerDocuments, SUCCESS_MESSAGES.FOLLOW.FOLLOWERS_FETCHED, true, STATUS_CODES.OK));
    };
    /**
     ...
     */
    HandleGetFollowings = async (req, res) => {
        ...
        return res.status(STATUS_CODES.OK).json(new ApiResponse(followingDocuments, SUCCESS_MESSAGES.FOLLOW.FOLLOWINGS_FETCHED, true, STATUS_CODES.OK));
    };
}
